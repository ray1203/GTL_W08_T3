...
void SLevelEditor::BindEditorInput()
{
    ...
    FDelegateHandle Handle = Handler->OnMouseDownDelegate.AddLambda([this](const FPointerEvent& InMouseEvent)
        {
    ...
    InputDelegateHandles.Add(Handle);

    Handle = Handler->OnMouseMoveDelegate.AddLambda([this](const FPointerEvent& InMouseEvent)
        {
            if (ImGui::GetIO().WantCaptureMouse) return;
    ...
    Handle = Handler->OnMouseUpDelegate.AddLambda([this](const FPointerEvent& InMouseEvent)
        {
            if (ImGui::GetIO().WantCaptureMouse) return;
    ...
    Handle = Handler->OnRawMouseInputDelegate.AddLambda([this](const FPointerEvent& InMouseEvent)
        {
    ...
    Handle = Handler->OnMouseWheelDelegate.AddLambda([this](const FPointerEvent& InMouseEvent)
        {
            if (ImGui::GetIO().WantCaptureMouse) return;
    ...
    Handle = Handler->OnKeyDownDelegate.AddLambda([this](const FKeyEvent& InKeyEvent)
        {
    ...
    Handle = Handler->OnKeyUpDelegate.AddLambda([this](const FKeyEvent& InKeyEvent)
        {
            ActiveViewportClient->InputKey(InKeyEvent);
        });
    InputDelegateHandles.Add(Handle);
...
void SLevelEditor::UnbindEditorInput()
{
    FSlateAppMessageHandler* Handler = GEngineLoop.GetAppMessageHandler();
